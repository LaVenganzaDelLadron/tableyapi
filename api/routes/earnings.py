...
@router.get("/")
async def list_earnings(db: Session = Depends(get_db), current_user=Depends(require_admin)):
    data = index(db)
    return success("Earnings fetched successfully", data)


@router.post("/")
async def create_earning(payload: Earnings, db: Session = Depends(get_db), current_user=Depends(require_admin)):
