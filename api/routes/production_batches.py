...
from services.product_batches_service import (
    ...
    store as product_batches_store,
)
...
async def store(payload: ProductionBatches, db: Session = Depends(get_db), current_user = Depends(get_current_user)):
    data = product_batches_store(db, payload.raw_id, payload.roast_date, payload.milled_weight, payload.packaged_pieces, payload.production_cost)
