...
2. **Standard API response wrapper (Pydantic)**
   - Add a Pydantic schema for the response envelope (in `api/schemas/common.py`) and reuse the existing `api/responses.py` helpers to construct it.
   - Envelope spec:
     - `message: str`
     - `data: Any | None`
     - `error: ErrorPayload | None` where `ErrorPayload` has:
       - `code: str`
       - `detail: str`
   - Validation rules:
   ...
   - Integration rule:
     - All routes must return this envelope via shared helpers so response shape is consistent across the API.
